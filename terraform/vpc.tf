...
module "vpc" {
  ...
  # - a Main with no name and no subnets associated only 1 route : 
  #   - 10.0.0.0/16 target local
  # - another RT with 2 explicit subnets association (the public_subnets below) with 2 routes :
  #   - 10.0.0.0/16 target local
  #   - 0.0.0.0/0   target igw-... (igw, so it's a public subnet)
  # - another RT with 1 explicit subnet association (the private_subnets below) with 1 route :
  #   - 10.0.0.0/16 target local
